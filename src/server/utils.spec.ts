import { convertAmountToBn, isDockerInternalIP } from "./utils";
...
describe("utils", () => {
  ...
  [
    ...
    { ip: "172.31.0.3", expected: true },
    { ip: "::ffff:172.31.0.3", expected: true },
  ].forEach((t) =>
    test(`isDockerInternalIP ${String(t.ip)} => ${String(t.expected)}`, () => {
      expect(isDockerInternalIP(t.ip)).toEqual(t.expected);
    }),
  );
  ...
  [
    ...
    test(`isDockerInternalIP '${String(t.ip)}' => throws`, () => {
      expect(() => isDockerInternalIP(t.ip)).toThrow("Unrecognized format of IP");
    }),
  );
});
