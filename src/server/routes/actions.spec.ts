...
import {BotRequestType, DripRequestType, FaucetRequestType} from "../../types";

type optTypes = ({ external: true; recaptcha: string } & Omit<DripRequestType, "sender">)
...
jest.mock("../config", () => {
  const configMock: { [Key: string]: string } = {
    "RPC_ENDPOINT": "http://localhost",
    "INJECTED_TYPES": "{}",
    "FAUCET_ACCOUNT_MNEMONIC": "scrub inquiry adapt lounge voice current manage chief build shoot drip liar head season inside"
  };
  return {
    config:
      {
...
describe("/drip/* tests", () => {
  ...
  describe("/drip/web", () => {
    ...
    test("should request drip", async () => {
      ...
      expect(mockHandleRequest).toHaveBeenCalledWith(expect.objectContaining({
        external: true,
        address: "example1",
        recaptcha: "captcha1"
      }));
